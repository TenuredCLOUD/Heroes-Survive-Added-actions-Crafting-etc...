//IED crafting by: TenuredCLOUD v0.1

player removeAction playerIEDLcraftaddaction;
sleep 0.1;

playerIEDLcraftaddaction = player addAction ["<t size='1.0' font='RobotoCondensed'>Craft Large IED</t>",{

[] spawn {
player getVariable ["IEDLcraftActivated",1,true];
  _Check_magazines = magazines player;
  _Check_items = items player;
if ("herl_ma_wire" in _Check_items && "herl_ma_wirecu" in _Check_items && "herl_o_pliers" in _Check_items && "herl_ma_pager" in _Check_items && "herl_mb_battery" in _Check_items && "herl_o_carbattery_a" in _Check_items && "herl_o_screwd_b" in _Check_items && "herl_o_screwd_a" in _Check_items
...
sleep 5;
player getVariable ["IEDLcraftActivated",0,true];
} else {
